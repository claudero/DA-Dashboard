...
function listBuckets(oAuth2TwoLegged, token, callback) {
    ...
    async.waterfall([
        (next) => {
            BucketsAPI.getBuckets({limit : 100}, oAuth2TwoLegged, token).then(
                (response) => {
                    console.log("buckets:");
                ...
                (err) => {
                    if (err.statusCode === 404) {
                        console.log("Could not fetch bucket list");
 ...
 */
function queryBuckets(clientId, clientSecret, environment, callback) {
    if (environment) {
    ...
    async.waterfall([
        ...
        (next) => {
            listBuckets(oAuth2TwoLegged, token, next);
        }
    ], (err) => {
        if (err) {
...
if (require.main === module) {
    ...
    queryBuckets(commander.clientId, commander.clientSecret, commander.environment, (err) => {
            if (err) {
                process.exit(1);
            }
            process.exit();
